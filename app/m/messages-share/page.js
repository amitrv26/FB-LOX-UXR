...
export default function MessagesSharePage() {
  const router = useRouter();
  const [phase, setPhase] = useState(0);
  const [bottomSheetOpen, setBottomSheetOpen] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState('videoLinkShare');
  const chatAreaRef = useRef(null);

  // Prefetch the destination page for instant navigation
  useEffect(() => {
    router.prefetch(`/m/reels/${sharedVideo.id}`);
  }, [router]);

  // Preload the video for instant playback when user taps
  useEffect(() => {
    ...
    link.rel = 'preload';
    link.as = 'video';
    link.href = sharedVideo.videoSrc;
    ...
    video.preload = 'auto';
    video.muted = true;
    video.src = sharedVideo.videoSrc;
    video.load();
  ...
  const handleVideoLinkTap = () => {
    // Store video data in sessionStorage for the reels page
    ...
    setTimeout(() => {
      router.push(`/m/reels/${sharedVideo.id}?source=google`);
    }, 1000);
  };
